    ...
    public class SchoolFees
    {
        ...
        public DataSet GetFeeMonthWise(string command,FeeCollection feeCollection)
        {
            using (SqlConnection con = new SqlConnection(constr))
            {
                try
                {
                    ...
                    com.Parameters.AddWithValue("@intStudent_id", feeCollection.intStudentID);
                    ...
                    else if (command == "PayFeeMonthWise")
                    {
                        ...
                        for (int i = 0; i < values.Length; i++)
                        {
                            ...
                            Monthfee.Tables.Add(MonthWiseFeeDetails(olddt, values[i], "MonthFeeDetails", ""));
                            Monthfee.Tables[0].TableName = "MonthFeeDetails";
        ...
        public DataSet GetPaidFeeMonthWise(string command, FeeCollection feeCollection)
        {
            using (SqlConnection con = new SqlConnection(constr))
            {
                try
                {
                        ...
                        if (StudentStartDate != "" && StudentEndDate != "")
                        {
                            ...
                            for (int i = 0; ds1.Tables[0].Rows.Count > i; i++)
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        BindDataWithStatus(Convert.ToString(feeCollection.intstandard_id), Convert.ToString(ds1.Tables[0].Rows[i]["intFeemaster_id"]), Convert.ToString(feeCollection.intSchool_id), StudentStartDate, StudentEndDate, feeCollection.intStudentID, Convert.ToString(ds.Tables[0].Rows[0]["Status"]));
                                    }
                                }
                                ...
                                if (ds.Tables[0].Rows.Count > 0)
                                {
                                    ...
                                    if (dsTransport.Tables[0].Rows.Count > 0)
                                    {
                                        ...
                                        else
                                        {
                                            if (Convert.ToDateTime(dtdate2) >= Convert.ToDateTime(StudentStartDate) && Convert.ToDateTime(dtdate2) <= Convert.ToDateTime(StudentEndDate))
                                            {
                                                dsTransport.Tables[0].Rows[0]["Status"] = Convert.ToString(ds1.Tables[0].Rows[i]["Status"]); // "Paid"
                    ...
                    else if(command == "PaidUnpaidMonth")
                    {
                        Monthfee.Tables.Add( PaidUnapaidMonths(feeCollection.intStudentID, feeCollection.intSchool_id).Copy());
                        Monthfee.Tables[0].TableName = "MonthWiseFee";
                    }
                    else if(command == "MonthWiseTotalPaidFee")
                    {
                        Monthfee.Tables.Add(MonthWiseCalculation(olddt, "MonthWiseTotalPaidFee"));
                        Monthfee.Tables[0].TableName = "MonthWiseFee";
                    }
                ...
                for (int i = 0; monthdiff >= i; i++)
                {
                    ...
                    else
                    {
                        if (Convert.ToDateTime(strFromDate) >= Convert.ToDateTime(StudentStartDate) && Convert.ToDateTime(strFromDate) <= Convert.ToDateTime(StudentEndDate))
                        {
                            CheckLateFee(StudStartDate, intstandard_id, intFeemaster_id, intschool_id);
                            dsObj.Tables[0].Rows[0]["month"] = GetMonths[i];
                            dsObj.Tables[0].Rows[0]["Status"] = ds2.Tables[0].Rows[0]["Status"]; //"Paid";
                            dsObj.Tables[0].Rows[0]["feestartdate"] = Convert.ToDateTime(strFromDate).ToString("MM/dd/yyyy");
                            GetConcession(strFromDate, intStudentID);
                            AddData(dsObj);
                        }
        ...
        private DataTable MonthWiseCalculation(DataTable dt, string command)
        {
            try
            {
                ...
                {
                    var result = (from row in dtCloned.AsEnumerable()
                                  where row.Field<string>("Status") == "Paid"
                                  group row by new
                                  {
                                      Month = row.Field<string>("month"),
                ...
                else if (command == "MonthWiseTotalUnPaidFee")
                {
                    var result = (from row in dtCloned.AsEnumerable()
                                  where row.Field<string>("Status") == "Unpaid"
                                  group row by new
                                  {
                                      Month = row.Field<string>("month"),
                                      Status = row.Field<string>("Status")
                ...
                if (command == "MonthFeeDetails")
                {
                    var Result = from MonthData in dt.AsEnumerable()
                                 ...
                                 select new
                                 {
                                     ...
                                     feestartdate = MonthData.Field<DateTime>("feestartdate")
                                 };
                    data = LINQResultToDataTable(Result);
                }
                else
        ...
        public DataSet InsertFeeRecords(string command, FeeCollection feeCollection)
        {
            try
            {
                ...
                strQry = "usp_Addfee @command='AddFeeinMaster',@intStudent_id='" + feeCollection.intStudentID + "',@intStandard_id='" + feeCollection.intstandard_id + "',@intDivision_id='" + feeCollection.intDivision_id + "',@intRoll_no='" + feeCollection.intRoll_no + "'" +
                    ",@vchDiscount='0',@vchNet_amt='" + sum + "',@vchRemark='Paid From Mobile APP',@ModeOfPayment='Mobile',@vchCheque_no='',@vchBank_name='',@intSchool_id='" + feeCollection.intSchool_id + "'" +
                        ",@intAcademic_id='" + feeCollection.intAcademic_id + "',@intInserted_by='" + feeCollection.intStudentID + "',@vchInserted_ip='" + feeCollection.vchInserted_ip + "',@Status='Inprogress',@PaidFrom='Mobile'";
                ...
                for (int i = 0; i < (dataset.Tables[0].Rows.Count); i++)
                {
                    strQry = "usp_Addfee @command='InsertFeeDetails',@intStudFee_id='" + ExcecuteQur + "',@intFeemaster_id='" + dataset.Tables[0].Rows[i]["intFeemaster_id"] + "',@intNoOf_month=''" +
                       ...
                       ",@intConcession_amt='" + dataset.Tables[0].Rows[i]["ConcessionAmt"] + "',@vchNetDetail_amt='" + dataset.Tables[0].Rows[i]["NetAmt"] + "',@intSchool_id='" + feeCollection.intSchool_id + "',@intAcademic_id='" + feeCollection.intAcademic_id + "',@vchStatus='Inprogress',@dtFee_date='" + dataset.Tables[0].Rows[i]["feestartdate"] + "'";
                    ReturnDataset = sGetDataset(strQry);
                }
                ReturnDataset.Tables[0].TableName = "PayFeeDetails";
        ...
        private DataTable PaidUnapaidMonths(int intStudent_id, int intSchool_id)
        {
            try
            {
                DataTable UnpaidMonth = new DataTable();
                UnpaidMonth= MonthWiseCalculation(olddt, "MonthWiseTotalUnPaidFee");

                DataColumnCollection col1 = UnpaidMonth.Columns;
                if (!col1.Contains("intStudFee_id"))
                    UnpaidMonth.Columns.Add("intStudFee_id", typeof(int));
                UnpaidMonth.Columns["intStudFee_id"].DefaultValue = 0;

                strQry = "usp_Addfee @command='GetPaidMonths',@intStudent_id='" +intStudent_id + "',@intSchool_id='"+intSchool_id+ "',@intStudFee_id='0'";
                DataSet ds1 = sGetDataset(strQry);

                DataTable paidrecords = ds1.Tables[0];

                // UnpaidMonth.Merge(ds1.Tables[0]);

                paidrecords.Merge(UnpaidMonth);
